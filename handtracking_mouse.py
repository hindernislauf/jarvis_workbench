...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            midpoint_x = (indexfinger_tip.x + thumb_tip.x) /2
            midpoint_y = (indexfinger_tip.y + thumb_tip.y) /2
            
            # Get the distance between the thumb and index finger
            distance = np.sqrt((index_finger_tip.x - thumb_tip.x)**2 + (index_finger_tip.y - thumb_tip.y))
